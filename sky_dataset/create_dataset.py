...
'''
path:文件名，即~.txt, 使用时请注意将文本文件放在~/sky_dataset/data_txt/目录下
'''
def get_names(path):
    '''从txt文本中获取图像的路径'''
...
def data_to_npz(path,filename):
    '''使用该函数直接将数据集保存为.npz格式'''
    x_dataset,t_dataset=create_dataset(path)
    np.savez_compressed(filename,x_dataset=x_dataset,t_dataset=t_dataset)
    return
